...
function MenuItems({ iconClass, itemName, itemRoute }: tabProps) {
    return (
            <NavLink to={`/${itemRoute}`} id={`menuItem ${itemName}`}  className={({ isActive }) => `flex flex-row items-start p-3.5 rounded-lg gap-2 ${isActive ? 'bg-blue-50 active_nav_link' : ""}`} title={itemName}>
